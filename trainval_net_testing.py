...
#Este es un sampler particular que funciona basicamente como un randomsampler
class sampler(Sampler):
  def __init__(self, train_size, batch_size):
    self.num_data = train_size #Cantidad total de datos
    self.num_per_batch = int(train_size / batch_size) #Numero de batches
    self.batch_size = batch_size 
    self.range = torch.arange(0,batch_size).view(1, batch_size).long() #Arreglo de numeros desde 0 hasta batch_size
    self.leftover_flag = False
    if train_size % batch_size: #Tratamiento del batch sobrante de tamaño menor al otro
      self.leftover = torch.arange(self.num_per_batch*batch_size, train_size).long()
      self.leftover_flag = True

  def __iter__(self):
    rand_num = torch.randperm(self.num_per_batch).view(-1,1) * self.batch_size #Permutación de enteros de 0 a la cantidad de batches total, 
    self.rand_num = rand_num.expand(self.num_per_batch, self.batch_size) + self.range #Generacion de indices restantes

    self.rand_num_view = self.rand_num.view(-1) #Lista de numeros aleatorios desde cero hasta num_per_batch*batch_size

    if self.leftover_flag:
      self.rand_num_view = torch.cat((self.rand_num_view, self.leftover),0) #Si hay resto, se agregan
...
if __name__ == '__main__':
  ...
  #Se selecciona la configuracion de acuerdo a net y goup
  ...
  #pprint.pprint(cfg)
  print('Utilzando random seed de configuracion:', cfg.RNG_SEED)
